...
class Ball:
    def __init__(self, speed, col): #random initialization
        self.x = random.randint(50,350) + random.choice((0,400))
        self.y = random.randint(50,550)
        phase1 = random.uniform(30,55)/180.0
        phase = (random.choice((1,2,3,4))/2.0 + phase1)*math.pi
...
def drawSplashScreen(): #draws the splash screen with four boxes
    ...
    canvas.create_oval(cd.width-cd.border,cd.border,cd.width-cd.border*2,
                       cd.border*2,fill="cyan", width=0)
    leftX=cd.width/2-cd.boxSize
    rightX=cd.width/2+cd.boxSize
    topY=cd.boxSize/2
    botY=cd.boxSize
    for incNum in xrange(cd.menus): #creates the rectangles for the splashscreen
...
def addBallPair():#adds a pair of balls(different colors) to the board
    ...
    cd.ballList.append(Ball(9.0,"cyan"))
...
def startGameDemo():#Creates game with a demo amount of balls
    cd.ballList=[]
    for numPairs in xrange(cd.demo):
        addBallPair()

def startGameNorm():#Creates game with a normal amount of balls
    cd.ballList=[]
    for numPairs in xrange(cd.norm):
        addBallPair()
...
def drawBoard():
    ...
    canvas.create_rectangle(0, cd.height-cd.wall, cd.width/2, cd.height,
                            fill="red", width=0)
    canvas.create_rectangle(cd.width/2,0,cd.width,cd.wall,fill="cyan", width=0)
